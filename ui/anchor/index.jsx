...
export const DarkAnchor = ({ children, link }) => {
	return (
		<a href={link} className='cursor-pointer'>
			<p
				className='font-montserrat inline-block text-base md:text-lg xl:text-xl font-bold text-black hover:text-cyan-400
                        after:block
                        after:border-b-2
                        after:border-cyan-400
                        after:scale-x-0
...
export const CardAnchor = ({ children, link }) => {
	return (
		...
		>
			<p className='w-full bg-white rounded-2xl border-solid border-2 border-orange-700 px-2 py-1 text-xs lg:text-sm font-bold text-orange-700 text-center flex justify-center items-center'>
				{children}
			</p>
		</a>
	)
